...
class HetMultiOutputEnsembleGP(torch.nn.Module):
    """
    -- Heterogeneous Multi Output Ensemble for Gaussian Processes --
    -- Accepts one channel x,y of data. --
    """

    def __init__(self, models, likelihood, kernels, Q, M, input_dim=None, batch_rate=1.0):
        super(HetMultiOutputEnsembleGP, self).__init__()
        ...
        self.D = self.K + 1  # the number of modules + data channel
        ...
        self.q_m = torch.nn.Parameter(torch.randn(M, Q), requires_grad=True)  # variational: mean parameter
    ...
    def expectation(self, x, y):
        ...
        for q in range(self.Q):
            ...
            q_L = torch.tril(self.q_L[:, :, q])
            q_S[:, :, q] = torch.mm(q_L, q_L.t())
        Kff = self.coregionalization.Kff(x, self.D-1)
        Kfu = self.coregionalization.Kfu(x, self.z, self.D-1)
        ...
        v_f = torch.diag(S_f)
        expectation_y = self.likelihood.variational_expectation(y, m_f, v_f)

        return E, expectation_y
    ...
    def forward(self, x, y):
        ...
        # Expectation --
        expectation, exp_y = self.expectation(x, y)
        expectation_y = self.batch_rate * exp_y.sum()

        # KL divergence --
        kl = self.divergence(q_u, p_u)

        # Calls ELBO
        elbo = expectation + expectation_y - kl
        return -elbo
